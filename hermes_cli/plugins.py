...
class PluginContext:
    ...
    def register_command(
        ...
        """
        在 CLI 和 Gateway 会话中注册可用的斜杠命令（例如 ``/lcm``）。

        处理函数的签名应为 ``fn(raw_args: str) -> str | None``。
        它也可以是异步可调用对象（Async Callable）—— Gateway 的分发机制同时支持这两种形式。

        与 ``register_cli_command()``（用于创建 ``hermes <subcommand>`` 终端命令）不同，
        本函数注册的是会话内的斜杠命令，供用户在对话期间调用。

        ``args_hint`` 是可选的简短字符串（例如 ``"<file>"`` 或 ``"dias:7 formato:json"``），
        Gateway 适配器会利用它来显示带有参数输入框的命令——例如 Discord 原生的斜杠命令选择器。
        若插件命令未提供 ``args_hint``，在 Discord 中将被注册为无参命令；
        但当作为自由文本对话调用时，它仍可接收后续的附加文本。

        若命令名称与内置命令冲突，系统将发出警告并拒绝注册。
        """
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        # 目录插件已在上方完成收集。
        # Pip / 入口点（entry-point）插件被特意独立分开：
        # 便携式软件包仅支持目录形式，
        # 且启动阶段的 MCP 探测切勿导入或注册入口点。
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        """调用所有针对 *hook_name* 注册的回调函数。

        钩子（Hook）负载采用增量演进方式。
        接受 ``**kwargs`` 的回调将接收完整的负载；
        而签名较窄的旧版回调则仅接收其显式声明的关键字参数。
        每个回调均被包裹在独立的 try/except 块中，
        以防止表现异常的插件破坏核心智能体（agent）循环。

        返回由回调函数返回的非 ``None`` 值构成的列表。

        对于 ``pre_llm_call``，回调可以返回一个描述要注入当前轮次用户消息的上下文的字典::

            {"context": "recalled text..."}
            "recalled text..."          # 纯字符串，效果等同

        上下文总是被注入到用户消息中，而绝不会注入到系统提示词（system prompt）中。
        这保留了提示词缓存前缀 ——
        系统提示词在多个轮次中保持不变，从而能够重用缓存的 token。
        所有注入的上下文都是临时的 —— 绝不会持久化到会话数据库中。
        """
        # 大多数遗留的观察者钩子都带有共享的遥测标记。
        # 而 Gateway 平台事件则改用局部的增量负载结构：
        # 如果在此处注入全局总线版本，会将原本不相关的适配器负载混为一个庞大的兼容性契约 (#64176)。
