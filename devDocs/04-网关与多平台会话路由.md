...
stateDiagram-v2
    ...
    state "以 SessionSource 生成会话键<br/>(平台+chat+user+thread [+profile])" as KeyLookup
    state "_should_reset() (session.py:1575)<br/>过期策略 / 显式 /new / 平台重置规则" as ResetCheck
    state "新会话<br/>生成 session_id, 建 SessionEntry" as New
    state "复用会话<br/>从 SessionDB 取历史 → agent_history" as Reuse
    state "_recover_session_from_db (:1348)<br/>进程重启后的会话找回<br/>(校验 profile 归属 :1295)" as Recover
    state "新鲜度闸门 (:29-57)<br/>resume_pending 距今 < 1h?" as ZombieGate
