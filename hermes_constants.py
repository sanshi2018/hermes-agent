...
def _hermes_home_from_env() -> Path:
    """
    仅仅从进程环境中解析 ``HERMES_HOME``。

    读取 ``HERMES_HOME`` 环境变量；如果未设置，则退回到平台原生默认值。
    此函数特意忽略了由 :func:`set_hermes_home_override` 安装的上下文局部重写，
    因此它反映的是进程/启动作用域，而非单任务配置文件。
    该逻辑由 :func:`get_hermes_home` 与 :func:`get_process_hermes_home` 共享，
    以确保两者永远不会发生偏离。
